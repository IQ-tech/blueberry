...
function inputedValueIsValid(value) {
...
export default function useTokenInput({
  ...
  function _handleValueEvents() {
    ...
    const stringFied = tokenValueArray.join("");
    const isTokenValid = stringFied.length === size;
    if (!!onChange) onChange(stringFied);

    if (!!isTokenValid && !!onCompleteToken) {
      onCompleteToken(stringFied);
    }
  }
  ...
  function _resetFocus() {
    const firstItem = rootElementRef.current.querySelector(
      `[data-token-i='0']`
    );
    firstItem.focus();
  }
  ...
  function onChangeNumber(e) {
    ...
    const isNumberValid = inputedValueIsValid(value);
    const isEmpty = value === "";
